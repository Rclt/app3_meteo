    ...
    render(){
    		return 	<div className='row inputs'>
    			<Input name='product' change={this.handleChange.bind(this, 'product')} value={this.state.product} placeholder='Produto' />
	...
	class AddButton extends React.Component {
		...
		onClick(){
			if(this.props.newData.product.length){
				this.props.create.call(null, this.props.newData.product);
    ...
    class Row extends React.Component {
    	render(){
    		return  <tr>
				<Column type='product' placeholder={this.props.product} />
				...
				<tbody>
					{
		    			this.props.datas.map((data, i) => {
		    				return <Row key={i} uuid={i} product={data.product} onDelete={this.props.onDelete} />
		    			})
	    			}
				</tbody>
    		</table>
    ...
    class Container extends React.Component {
    	...
    	onCreate(product){
    		this.setState({datas: this.state.datas.concat([{product: product}])});
    	}
